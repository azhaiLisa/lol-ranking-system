...
# Strategic importance weights for objectives
OBJECTIVE_WEIGHTS = {
    "DRAGON": 3.5,
    "BARON_NASHOR": 5.0,
    "RIFTHERALD": 2.5,
    "TOWER_BUILDING": 1.8,
    "INHIBITOR_BUILDING": 4.0,
    "NEXUS_TURRET": 5.0
}
...
def process_frame_states(frame: Dict) -> List[str]:
    ...
    for pid, data in frame["participantFrames"].items():
        tokens.extend([
            f"[STATE][P{pid}][GOLD][{data['totalGold']}]",
            f"[STATE][P{pid}][LEVEL][{data['level']}]",
            f"[STATE][P{pid}][CS][{data['minionsKilled'] + data['jungleMinionsKilled']}]",
            f"[STATE][P{pid}][ITEMS][{','.join(get_item_category(str(item)) for item in data['items'])}]"
        ])
    return tokens
...
def process_combat(event: Dict[str, Any]) -> List[str]:
    ...
    # Only mark as first blood if it's the first kill
    # I don't think this is needed because the firstblood will be handled in the specialkills
    # kill_type = "FB" if event.get("killStreakLength", 0) == 1 else "NORMAL"
...
def get_objective_weight(objective_type: str, sub_type: str = None) -> float:
    """Get the strategic weight of an objective."""
    # Handle special cases
    if objective_type == "TOWER_BUILDING" and sub_type:
        if "NEXUS" in sub_type:
            return OBJECTIVE_WEIGHTS["NEXUS_TURRET"]
        elif "INHIBITOR" in sub_type:
            return OBJECTIVE_WEIGHTS["INHIBITOR_BUILDING"]
    
    return OBJECTIVE_WEIGHTS.get(objective_type, 1.0)
...
def process_building(event: Dict[str, Any]) -> str:
    ...
    # weight = get_objective_weight(building_type, tower_type)
...
def process_elite_monster(event: Dict[str, Any]) -> str:
    ...
    # # Get strategic weight
    # weight = get_objective_weight(monster_type, monster_subtype)
    ...
    # return [{monster_subtype}][{participants_str}][W{weight}]"
    return tokens
...
if __name__ == "__main__":
    ...
    # match_file = "example_match.json"
    # tokens = process_match_file(match_file)
    # print("\n".join(tokens))
